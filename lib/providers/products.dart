...
class Products with ChangeNotifier {
  List<ProductItem> _items =
      []; //KITA INISIASI DATA AWAL KOSONG DENGAN TIPE LIST DAN DIDALAMNYA MEMILIKI FORMAT DATA SESUAI DENGAN PRODUCT ITEM.
  ...
  Future<void> fetchProduct() async {
    ...
    final response = await http.get(
        url); //MENGGUNAKAN AWAIT UNTUK MENUNGGU PROSESNYA SEBELUM MELANJUTKAN KE CODE SELANJUTNYA
    final convertData = json.decode(response.body) as Map<String,
        dynamic>; //DECODE DATA DAN UBAH FORMATNYA DENGNA FORMAT MAP DAN KEYNYA ADALAH STRING, VALUENYA DYNAMIC
...
//METHOD YG BERFUNGSI UNTUK MENGURANGI STOK BERDASARKAN ID
  Future<void> changeStock(String id) async {
    ...
    await http.patch(url,
        body: json.encode({'stock': stock})); //UPDATE DATA DI SERVER
...
//METHOD INI UNTUK MENGUPDATE SEMUA INFORMASI DATA YANG SEDANG DI EDIT
  Future<void> updateProduct(ProductItem product) async {
    ...
    final url =
        'https://dw-inventory.firebaseio.com/products/${product.id}.json';
...
//HAPUS PRODUK BERDASARKAN ID
  Future<void> removeProduct(String id) async {
    ...
    _items
        .removeWhere((prod) => prod.id == id); //DAN HAPUS JUGA PADA LOCAL STATE
    notifyListeners();
  }
}
